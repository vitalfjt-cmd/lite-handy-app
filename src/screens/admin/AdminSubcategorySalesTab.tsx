...
import { getStaffSubcategorySalesHistory, getStaffSalesReport } from '../../lib/staffReadApi'
...
export function AdminSubcategorySalesTab({ storeSlug, disabled, yen, setError }: Props) {
  ...
  const handleSearch = async () => {
    ...
    setError(null)
    try {
      const res = await getStaffSubcategorySalesHistory(storeSlug, startDate, endDate)
      setItems(res.items || [])
    } catch (err: any) {
  ...
  return (
    <div className="admin-tab-content" style={{ maxHeight: '100%', height: '100%', overflowY: 'auto', paddingRight: '12px', paddingBottom: '32px' }}>
      <div className="admin-tab-head">
        <div>
          <h2>サブカテゴリ別売上</h2>
          <p className="caption">期間を指定して、日付ごとのサブカテゴリ別売上金額を集計・照会します。</p>
        </div>
      </div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.95rem' }}>
              <thead>
                <tr style={{ background: '#f8f9fa' }}>
                  ...
                  <th style={{ padding: '12px 8px', textAlign: 'left', borderBottom: '2px solid #dee2e6', color: '#495057', position: 'sticky', top: 0, background: '#f8f9fa', zIndex: 1 }}>サブカテゴリ名</th>
                ...
                {items.length === 0 ? (
                  ...
                  items.map((item, idx) => (
                    <tr key={idx} style={{ borderBottom: '1px solid #f1f3f5' }}>
                      ...
                      <td style={{ padding: '12px 8px', color: '#212529' }}>{item.subcategory_name}</td>
                      <td style={{ padding: '12px 8px', textAlign: 'right', fontWeight: 'bold', color: '#212529' }}>{yen(item.amount)}</td>
                    </tr>
                  ))
                )}
              </tbody>
