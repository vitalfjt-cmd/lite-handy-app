...
test.describe('hex order smoke', () => {
  test('submits one order from six customer contexts', async ({ browser, baseURL }) => {
    ...
    test.skip(!scenario.customerOrderItemName, 'PW_CUSTOMER_ORDER_ITEM_NAME is required for hex smoke')
    ...
    try {
      ...
      if (scenario.staff) {
        ...
        await expect.poll(async () => await readStaffSelectedLineCount(staff.page), { timeout: 30_000 }).toBeGreaterThanOrEqual(6)
      }
    } finally {
