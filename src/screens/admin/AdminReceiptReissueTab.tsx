              ...
              <>
                ...
                <div 
                  ...
                  style={{ 
                    ...
                    overflow: 'hidden',
                    fontFamily: 'system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, "Hiragino Sans", "Hiragino Kaku Gothic ProN", sans-serif'
                  }}
                >
                  <div style={{ padding: '14px 14px 0', flexShrink: 0, boxSizing: 'border-box', width: '100%' }}>
                    ...
                    <h3 className="receipt-brand" style={{ textAlign: 'center', margin: '0 0 4px', fontSize: '1.15rem', fontFamily: 'inherit' }}>店舗デモ</h3>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: '0.85rem', gap: '8px' }}>
                      ...
                      <span style={{ flexShrink: 0, textAlign: 'right' }}>伝票番号: {ticketDetail.ticket_no}</span>
                    </div>
                    <div className="receipt-divider" style={{ borderTop: '1px dashed #ccc', margin: '6px 0' }}></div>
                  </div>

                  <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', padding: '0 14px', boxSizing: 'border-box', width: '100%' }}>
                    {ticketDetail.lines.map((line: any, idx: number) => {
                      const isReduced = line.tax_rate_type === 'REDUCED'
                      return (
                        <div key={idx} className="receipt-item-row" style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) auto auto', gap: '8px', alignItems: 'baseline', marginBottom: '6px', boxSizing: 'border-box', width: '100%' }}>
                          <span className="receipt-item-name" style={{ display: 'block', minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontSize: '0.88rem' }}>{line.item_name_snapshot}{isReduced ? ' ※' : ''}</span>
                          <span className="receipt-item-qty" style={{ color: '#666', fontSize: '0.85rem', flexShrink: 0, fontVariantNumeric: 'tabular-nums' }}>x{line.quantity}</span>
                          <span className="receipt-item-price" style={{ textAlign: 'right', fontWeight: 'bold', fontSize: '0.9rem', flexShrink: 0, paddingLeft: '4px', fontVariantNumeric: 'tabular-nums' }}>{yen(line.line_subtotal)}</span>
                        </div>
                      )
                    })}
                  </div>

                  <div style={{ padding: '0 14px 14px', borderTop: '2px dashed #aaa', marginTop: '6px', background: '#fafafa', borderRadius: '0 0 14px 14px', flexShrink: 0, boxSizing: 'border-box', width: '100%' }}>
                    <div className="receipt-grand-total" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', fontSize: '1.1rem', fontWeight: 'bold', margin: '8px 0 4px' }}>
                      <span>ご請求額</span>
                      <span className="total-amount" style={{ color: '#ff5a5f', fontSize: '1.3rem', fontWeight: 'bold', fontVariantNumeric: 'tabular-nums' }}>{yen(orderSubtotal)}</span>
                    </div>
                      ...
                      return (
                        <div style={{ marginTop: '4px', paddingTop: '4px', borderTop: '1px dashed #ccc', fontSize: '0.8rem', color: '#555' }}>
                          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                            ...
                            <span style={{ fontWeight: 600, fontVariantNumeric: 'tabular-nums' }}>{yen(total10)}</span>
                          </div>
                          <div style={{ display: 'flex', justifyContent: 'space-between', paddingLeft: '8px', color: '#777' }}>
                            <span>(内消費税額)</span>
                            <span style={{ fontVariantNumeric: 'tabular-nums' }}>{yen(tax10)}</span>
                          </div>
                          {total8 > 0 && (
                            <>
                              <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '2px' }}>
                                <span>{redRate}%対象金額(※)</span>
                                <span style={{ fontWeight: 600, fontVariantNumeric: 'tabular-nums' }}>{yen(total8)}</span>
                              </div>
                              <div style={{ display: 'flex', justifyContent: 'space-between', paddingLeft: '8px', color: '#777' }}>
                                <span>(内消費税額)</span>
                                <span style={{ fontVariantNumeric: 'tabular-nums' }}>{yen(tax8)}</span>
                              </div>
                            </>
                          )}
                        </div>
                    ...
                    {ticketDetail.payment_entries.map((entry: any, idx: number) => (
                      <div key={idx} style={{ marginBottom: '4px', fontSize: '0.85rem' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                          ...
                          <span style={{ fontWeight: 'bold', flexShrink: 0, paddingLeft: '4px', fontVariantNumeric: 'tabular-nums' }}>{yen(entry.received_amount || entry.final_amount)}</span>
                        </div>
                        {entry.change_amount > 0 && (
                          <div style={{ display: 'flex', justifyContent: 'space-between', color: '#666', fontSize: '0.8rem', paddingLeft: '12px', marginTop: '1px' }}>
                            <span>お釣り</span>
                            <span style={{ flexShrink: 0, fontVariantNumeric: 'tabular-nums' }}>{yen(entry.change_amount)}</span>
                          </div>
                        )}
                      </div>
                    ))}
