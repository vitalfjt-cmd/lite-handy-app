...
test.describe('quad order smoke', () => {
  test('submits one order from four customer contexts', async ({ browser, baseURL }) => {
    ...
    test.skip(!scenario.customerOrderItemName, 'PW_CUSTOMER_ORDER_ITEM_NAME is required for quad smoke')
    ...
    try {
      ...
      if (scenario.staff) {
        ...
        await expect.poll(async () => await readStaffSelectedLineCount(staff.page), { timeout: 30_000 }).toBeGreaterThanOrEqual(4)
      }
    } finally {
