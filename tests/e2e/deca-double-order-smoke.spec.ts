...
test.describe('deca double order smoke', () => {
  test('submits two orders from ten customer contexts', async ({ browser, baseURL }) => {
    ...
    test.skip(!scenario.customerOrderItemName, 'PW_CUSTOMER_ORDER_ITEM_NAME is required for deca double smoke')
    ...
    try {
      ...
      const additionalCustomers = await Promise.all([
        ...
        openCustomerContext(createContext, customerUrl, 9),
        openCustomerContext(createContext, customerUrl, 10),
      ])
      ...
      if (scenario.staff) {
        ...
        await expect.poll(async () => await readStaffSelectedLineCount(staff.page), { timeout: 30_000 }).toBeGreaterThanOrEqual(20)
