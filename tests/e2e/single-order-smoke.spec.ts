...
test.describe('single order smoke', () => {
  test('submits one customer order with optional staff verification', async ({ browser, baseURL }) => {
    ...
    try {
      ...
      const customer = await openCustomerContext(createContext, customerUrl, 1)
      let hasDirectOrderingPath = await hasOrderableCustomerItem(customer.page)
      ...
      const itemName = scenario.customerOrderItemName ?? (await discoverOrderableCustomerItemName(customer.page))
      await addOneItemAndOpenConfirm(customer.page, itemName)
      const timing = await submitOrder(customer.page, 0, 1)

      expect(timing.responseStatus).toBe(200)
      expect(timing.totalMs).toBeLessThanOrEqual(scenario.maxTotalMs ?? 5_000)
      ...
      if (scenario.staff) {
        ...
        const targetTableLabel = await readCustomerActiveTableLabel(customer.page)
        ...
        await expect.poll(async () => await readStaffSelectedLineCount(staff.page), { timeout: 30_000 }).toBeGreaterThan(0)
      }
    } finally {
